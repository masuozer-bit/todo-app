    ...
    <div className="space-y-4">
      ...
      {stats.accuracyScore !== null && (
        <div className="rounded-xl bg-white dark:bg-black px-3 py-2.5">
          <div className="flex items-center justify-between mb-1.5">
            <span className="text-[11px] font-medium text-gray-400 uppercase tracking-wide">Estimation Accuracy</span>
      ...
      {!hasData && (
        <div className="text-center py-6">
          ...
          <p className="text-[11px] text-gray-600 mt-0.5">Click the play button on a task to start</p>
        </div>
      )}
    </div>
  );
...
function ListBreakdown({ title, items, max }: { title: string; items: { name: string; color: string | null; seconds: number }[]; max: number }) {
  return (
    <div className="rounded-xl bg-white dark:bg-black px-3 py-2.5">
      <p className="text-[11px] font-medium text-gray-400 uppercase tracking-wide mb-2">{title}</p>
        ...
        {items.map((item) => (
          <div key={item.name}>
            <div className="flex items-center justify-between mb-0.5">
              ...
              <span className="text-[11px] text-gray-500 tabular-nums">{formatDuration(item.seconds)}</span>
            </div>
